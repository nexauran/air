...
/**
 * Local shape for product item we might receive from Sanity.
 * We keep fields loose (any) because server shapes vary.
 */
type AnyImage = any;
type AnyProduct = any;
...
const OrderDetailDialog: React.FC<OrderDetailsDialogProps> = ({
  ...
  // Normalize items once (referenced product or inline object)
  const normalizedItems = useMemo(() => {
    const raw = Array.isArray(order.products) ? (order.products as any[]) : [];
    return raw.map((it: any, idx: number) => {
      const qty = Number(it?.quantity ?? 1);

      // product-> expanded reference (if available)
      let prod: AnyProduct | null = it?.product ?? null;

      // if there's no expanded ref, build product-like object from inline fields (free gift shape)
      if (!prod || Object.keys(prod).length === 0) {
        ...
        prod = {
          ...
          // keep raw meta if present
          meta: it.meta ?? undefined,
        };
      } else {
        // ensure numeric price and keep any flags
...
const displayDiscount = typeof order.amountDiscount === "number" ? order.amountDiscount : 0;
  // recompute subtotal using normalizedItems
  const computedSubtotal = useMemo(() => {
    if (typeof (order as any).subtotal === "number") return (order as any).subtotal;
  ...
  const displayShipping =
    typeof (order as any).shippingCharge === "number"
  ...
  const displayTotal =
    typeof order.totalPrice === "number" ? order.totalPrice : Math.round(productsTotal + displayShipping);

  // Build WhatsApp message using normalized items
      ...
      normalizedItems
        .map((it) => {
          const name =
            ...
            it.raw?.name ??
            it.raw?.title ??
            (it.product?.slug?.current ? `Product ${it.product.slug.current}` : "Free item");
          const qty = it.quantity ?? 1;
          const priceValue = (typeof it.product?.price === "number" ? it.product.price : Number(it.product?.price ?? 0)) * qty;
...
Customer: ${order.customerName ?? order.email ?? ""}`;
  ...
  }, [order, normalizedItems, computedSubtotal, productsTotal, displayShipping, displayDiscount, displayTotal]);
              ...
              <div className="space-y-3">
                {normalizedItems.length ? (
                  normalizedItems.map((it, idx) => {
                    ...
                    // resolve display name with sensible fallbacks
                    const displayName =
                      prod.name ??
                      prod.title ??
                      it.raw?.name ??
                    ...
                    // build image safely: prefer asset.url, else use urlFor
                    const imgCandidate: AnyImage | null =
                      Array.isArray(prod.images) && prod.images.length ? prod.images[0] : null;

                    let imgUrl = "";
                    try {
                      if (imgCandidate && typeof imgCandidate === "object") {
                        const asset = (imgCandidate as any).asset;
                        if (asset && typeof asset === "object" && typeof asset.url === "string" && asset.url) {
                          imgUrl = asset.url;
                        } else {
                          imgUrl = urlFor(imgCandidate).url();
                        }
                    ...
                    return (
                      <div key={idx} className="flex items-center gap-4 p-3 rounded-lg border bg-gray-50">
                        ...
                        <div className="flex-1 flex items-center justify-between gap-4">
                          ...
                          <div className="text-right">
                            ...
                            <div className="text-sm font-semibold">
                              <PriceFormater amount={Number(lineTotal)} />
                            </div>
                          </div>
                        </div>
                      </div>
